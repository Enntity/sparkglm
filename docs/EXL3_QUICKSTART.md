...
Historical EXL3 video foundation. For the current NVFP4 default see [the quickstart](../SPARKGLM.md).
...
A fresh checkout builds the root Dockerfile automatically. `BUILD=1 ./start-exl3.sh`
...
| Weight load/graph capture | `./start-exl3.sh logs` and `./start-exl3.sh logs worker` |
...
them with `./start-exl3.sh status`, then use `./start-exl3.sh stop` if abandoning the
attempt.
...
```bash
./start-exl3.sh status
./start-exl3.sh logs
./start-exl3.sh logs worker
./start-exl3.sh stop
./start-exl3.sh restart
```
