...
root=pathlib.Path(sys.argv[1]);sets=json.loads((root/'comparison.json').read_text());fps=12;W,H=2560,780
...
out=root/'mia-latest-vs-sparkglm-nvfp4-c4.mp4'
...
for frame in range(math.ceil(end*fps)):
 ...
 d.text((30,20),'Latest Mia EXL3 vs SparkGLM NVFP4 · staggered C4',font=F[38],fill='#f3f6fb')
 ...
 for c,item in enumerate(sets):
  a=item['capture'];x=30+(c%2)*1270;oy=(c//2)*640;color=['#5fe0ae','#92a8c9'][c]
 ...
 d.text((30,710),'Mia: as-shipped skip scheduling. SparkGLM: current NVFP4 recipe. Median of 3 runs each; configurations differ.',font=F[20],fill='#a8b9cb')
 d.text((30,745),'Real SSE arrival times. Token animation uses server totals distributed across text deltas. Full elapsed time retained.',font=F[18],fill='#8296ac')
