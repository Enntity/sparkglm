...
# Adapted from the September 8 SparkGLM measured-SSE renderer; no synthetic timing.
import json,pathlib,math,subprocess,textwrap,sys
from PIL import Image,ImageDraw,ImageFont
root=pathlib.Path(sys.argv[1]);sets=json.loads((root/'panels.json').read_text());fps=12;W,H=1280,1400
...
out=root/'comparison-stacked.mp4'
...
for frame in range(math.ceil(end*fps)):
 ...
 d.text((30,20),'SparkGLM · NVIDIA vs Red Hat',font=F[38],fill='#f3f6fb')
 ...
 for c,item in enumerate(sets):
  a=item['capture'];x=30;oy=c*600;color=['#5fe0ae','#92a8c9'][c]
 ...
 d.text((30,1320),'Same SparkGLM images and DFlash2 k7. Median of 3 per checkpoint; blocked run order.',font=F[20],fill='#a8b9cb')
 d.text((30,1355),'Real SSE arrival times. Token animation uses server totals distributed across text deltas. Full elapsed time retained.',font=F[18],fill='#8296ac')
