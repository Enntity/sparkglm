...
# ./start-exl3.sh will rsync to the worker on launch unless SKIP_SYNC=1.
#
# Already complete? Exits 0. Re-fetch: REFRESH_WEIGHTS=1 ./download.sh
# Skip DFlash2: SPEC_METHOD=mtp ./download.sh
#
# Equivalent to: ./start-exl3.sh download
exec "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/start-exl3.sh" download
