...
if [[ "${ENFORCE_EAGER:-0}" == "1" ]]; then
  ...
  args+=(--cudagraph-capture-sizes 1 2 3 4 5 6 8 9 10 12 15 16 18 20 21 24 25 30 32 35 36 40 42 48)
fi

if [[ "${SPARKGLM_NVME_PREFIX_CACHE:-0}" == "1" ]]; then
  : "${SPARKGLM_NVME_ROOT:?Set a persistent private NVMe mount}"
  : "${SPARKGLM_NVME_NAMESPACE:?Pin checkpoint, serving build, TP and cache layout}"
  [[ "${PYTORCH_CUDA_ALLOC_CONF:-}" != *expandable_segments:True* ]] || {
    log "NVMe offloading requires expandable_segments disabled"; exit 1;
  }
  [[ -f /usr/local/lib/python3.12/dist-packages/vllm/v1/kv_offload/nvme_direct.py ]] || {
    log "selected image lacks the experimental NVMe connector"; exit 1;
  }
  export PYTHONHASHSEED=0
  nvme_config="$(python3 -S - <<'PYNVME'
import json, os
e=os.environ
print(json.dumps({"kv_connector":"OffloadingConnector", "kv_role":"kv_both", "kv_connector_extra_config":{
    "spec_name":"NvmeDirectOffloadingSpec", "spec_module_path":"vllm.v1.kv_offload.nvme_direct",
    "root_dir":e["SPARKGLM_NVME_ROOT"], "namespace":e["SPARKGLM_NVME_NAMESPACE"],
    "capacity_bytes":int(e.get("SPARKGLM_NVME_CAPACITY_BYTES",str(64*1024**3))),
    "reserve_bytes":int(e.get("SPARKGLM_NVME_RESERVE_BYTES",str(64*1024**3))),
    "n_io_threads":int(e.get("SPARKGLM_NVME_IO_THREADS","2")), "blocks_per_chunk":1}}))
PYNVME
)"
  args+=(--kv-transfer-config "$nvme_config" --enable-prompt-tokens-details)
fi
