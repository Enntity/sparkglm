...
def _validate_events(events):
    ...
    for index, event in enumerate(events):
        ...
        start = _finite_number(event["start"], label + ".start")
        end = _finite_number(event["end"], label + ".end")
        if end < start:
...
def _validate_windows(windows):
    ...
    for index, window in enumerate(windows):
        ...
        start = _finite_number(window["start"], label + ".start")
        end = _finite_number(window["end"], label + ".end")
        if not (end > start):
...
def _summarize_events(events, windows):
...
def summarize_events(events, windows):
    try:
        return _summarize_events(events, windows)
    except OverflowError as exc:
        raise ValueError("computed duration exceeds finite numeric range") from exc
