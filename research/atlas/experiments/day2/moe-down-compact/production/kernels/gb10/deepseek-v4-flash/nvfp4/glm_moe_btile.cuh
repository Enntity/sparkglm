...
// Direct-staged B variant of production M16 (502cda12); serving remains disabled.
// Include after the production native-FP4 grouped .cu for its primitives.
// ONLY B layout/staging differs; A/scales/K64 MMA order are unchanged.
// B is [N/128,K/64,128,32 packed bytes], equal size to original [K/2,N].
// CPU/native fixtures include this production single source of truth.
#pragma once

template<bool PQ_VEC_SCALES>
__device__ __forceinline__ void glm_moe_btile_impl(
    ...
    __shared__ unsigned char btile_Ap[2][16][K_STEP_T64 / 2 + 16];
    __shared__ unsigned char btile_As[2][16][K_STEP_T64 / GROUP_SIZE];
    // Already MMA-ready B rows: double buffer without runtime transpose.
    // Padding48 keeps each destination row16-byte aligned, as in production.
    __shared__ unsigned char btile_Bp[2][N_TILE_LG][K_STEP_T64 / 2 + 16];
    __shared__ unsigned char btile_Bs[2][K_STEP_T64 / GROUP_SIZE][N_TILE_LG];
    __shared__ int btile_tok[16];

    if (threadIdx.x < 16) {
        const int local_row = threadIdx.x;
        if (sorted_token_ids && (cta_m_local + local_row) < (unsigned int)M_expert)
            btile_tok[local_row] = sorted_token_ids[cta_m + local_row];
        else
            btile_tok[local_row] = (int)(cta_m + local_row);
    }
    __syncthreads();
    ...
    #define PQ4_ISSUE_LOADS(buf, kb) do { \
        if (threadIdx.x < 32) { \
            ...
            unsigned int a_row = (unsigned int)btile_tok[row]; \
            moe_cp_async_pred_16(&btile_Ap[(buf)][row][col], \
        ...
        if constexpr (PQ_VEC_SCALES) { \
            ...
            if (threadIdx.x < 16) { \
                ...
                unsigned int a_row = (unsigned int)btile_tok[row]; \
                moe_cp_async_pred_4(&btile_As[(buf)][row][0], \
        ...
        { \
            const unsigned long long tile = \
                ((unsigned long long)(cta_n / 128) * (K / 64) + (kb) / 64) * 4096; \
            _Pragma("unroll") \
            for (int rnd = 0; rnd < 2; rnd++) { \
                /* Contiguous16B per lane;128 threads cover128 rows x32B. */ \
                unsigned int linear = (rnd * 128 + threadIdx.x) * 16; \
                unsigned int bn = linear / 32, bk = linear % 32; \
                moe_cp_async_pred_16(&btile_Bp[(buf)][bn][bk], \
                    &B_expert[tile + linear], \
                    cta_n + bn < N && (kb) + bk * 2 + 31 < K); \
            } \
        } \
        if constexpr (PQ_VEC_SCALES) { \
            ...
            if (threadIdx.x < 32) { \
                ...
                moe_cp_async_pred_16(&btile_Bs[(buf)][g][ns], \
    ...
    #define PQ4_COMPUTE_MMA(a_buf, b_buf) do { \
        ...
        unsigned int a0 = PQ4_FRAG(btile_Ap[(a_buf)], ra,     tid * 8); \
        unsigned int a1 = PQ4_FRAG(btile_Ap[(a_buf)], ra + 8, tid * 8); \
        unsigned int a2 = PQ4_FRAG(btile_Ap[(a_buf)], ra,     32 + tid * 8); \
        unsigned int a3 = PQ4_FRAG(btile_Ap[(a_buf)], ra + 8, 32 + tid * 8); \
        unsigned int sfa_m = (lane_id & 1) * 8 + (lane_id >> 2); \
        unsigned int sfa = (unsigned int)btile_As[(a_buf)][warp_m_offset + sfa_m][0] \
            | ((unsigned int)btile_As[(a_buf)][warp_m_offset + sfa_m][1] << 8) \
            | ((unsigned int)btile_As[(a_buf)][warp_m_offset + sfa_m][2] << 16) \
            | ((unsigned int)btile_As[(a_buf)][warp_m_offset + sfa_m][3] << 24); \
        _Pragma("unroll") \
        for (int nt = 0; nt < 4; nt++) { \
            unsigned int nc = warp_n_offset + nt * 8 + group_id; \
            unsigned int b0 = PQ4_FRAG(btile_Bp[(b_buf)], nc, tid * 8); \
            unsigned int b1 = PQ4_FRAG(btile_Bp[(b_buf)], nc, 32 + tid * 8); \
            unsigned int sfn = warp_n_offset + nt * 8 + (lane_id >> 2); \
            unsigned int sfb = (unsigned int)btile_Bs[(b_buf)][0][sfn] \
                | ((unsigned int)btile_Bs[(b_buf)][1][sfn] << 8) \
                | ((unsigned int)btile_Bs[(b_buf)][2][sfn] << 16) \
                | ((unsigned int)btile_Bs[(b_buf)][3][sfn] << 24); \
...
template<bool VEC>
__device__ __forceinline__ void glm_moe_gate_up_btile_impl(GLM_GU_BTILE_ARGS) {
    ...
    glm_moe_btile_impl<VEC>(A_packed, A_scale,
...
#define GLM_GU_BTILE_CALL(VEC) \
    glm_moe_gate_up_btile_impl<VEC>(A_packed, A_scale, gate_packed_ptrs, gate_scale_ptrs, \
...
extern "C" __global__ void glm_moe_gate_up_btile(GLM_GU_BTILE_ARGS) {
    GLM_GU_BTILE_CALL(false);
}
extern "C" __global__ void glm_moe_gate_up_btile_vecscale(GLM_GU_BTILE_ARGS) {
    GLM_GU_BTILE_CALL(true);
}
#undef GLM_GU_BTILE_ARGS
#undef GLM_GU_BTILE_CALL
