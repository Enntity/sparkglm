    ...
    do { \
        for (unsigned int _i=(t); _i<TILE_CHUNKS_512; _i+=(stride)) { \
            ...
            if (_token>=0) { \
                ...
                glm_split_kvp_cp16(&(smem_ptr)[_row*GLM_KVP_STRIDE+_col], (cache)+_offset); \
...
__device__ __forceinline__ void glm_split_kvp_cp16(void* smem_dst, const void* gmem_src) {
    unsigned _s = __cvta_generic_to_shared(smem_dst);
    asm volatile("cp.async.cg.shared.global [%0], [%1], 16;" :: "r"(_s), "l"(gmem_src));
}
__device__ __forceinline__ void glm_split_kvp_cp16_pred(void* smem_dst, const void* gmem_src, bool pred) {
    unsigned _s = __cvta_generic_to_shared(smem_dst);
    unsigned _b = pred ? 16u : 0u;
    asm volatile("cp.async.ca.shared.global [%0], [%1], 16, %2;" :: "r"(_s), "l"(gmem_src), "r"(_b));
}
__device__ __forceinline__ void glm_split_kvp_cp_commit() { asm volatile("cp.async.commit_group;"); }
__device__ __forceinline__ void glm_split_kvp_cp_wait()   { asm volatile("cp.async.wait_group 0;"); }
...
__device__ __forceinline__ float glm_split_kvp_exp(float x) {
...
extern "C" __global__ void atlas_sparse_decode_split(
    ...
    for (unsigned int kv_block = first_block; kv_block < end_block; kv_block++) {
        ...
        if (warp_id < 2) {
            ...
            if (mn0 != m_r0) {
                float eo0=glm_split_kvp_exp(m_r0-mn0); l_r0*=eo0;
                #pragma unroll
            ...
            if (mn1 != m_r1) {
                float eo1=glm_split_kvp_exp(m_r1-mn1); l_r1*=eo1;
                #pragma unroll
            ...
            for(int nt=0;nt<4;nt++){
                ...
                float p00=valid0 ? glm_split_kvp_exp(acc_s[nt][0]-m_r0) : 0.0f;
                float p01=valid1 ? glm_split_kvp_exp(acc_s[nt][1]-m_r0) : 0.0f;
                float p10=valid0 ? glm_split_kvp_exp(acc_s[nt][2]-m_r1) : 0.0f;
                float p11=valid1 ? glm_split_kvp_exp(acc_s[nt][3]-m_r1) : 0.0f;
        ...
        if(warp_id>=2){
            ...
            if (cm0 != m_r0) {
                float er0=glm_split_kvp_exp(m_r0-cm0);
                #pragma unroll
                for(int i=0;i<N_TILES_PER_WARP_512;i++){acc_o[i][0]*=er0;acc_o[i][1]*=er0;}
                m_r0=cm0;
            }
            if (cm1 != m_r1) {
                float er1=glm_split_kvp_exp(m_r1-cm1);
                #pragma unroll
    ...
    // === Final normalization and store ===
    {
        ...
        for(int nt=0;nt<N_TILES_PER_WARP_512;nt++){
            ...
            if(gr0<q_len && r0<q_tile_len && c0<head_dim){
                ob[gr0*q_seq_stride+c0] = acc_o[nt][0]*il0;
                ob[gr0*q_seq_stride+c0+1] = acc_o[nt][1]*il0;
            }
            if(gr1<q_len && r1<q_tile_len && c0<head_dim){
                ob[gr1*q_seq_stride+c0] = acc_o[nt][2]*il1;
                ob[gr1*q_seq_stride+c0+1] = acc_o[nt][3]*il1;
            }
        }
    }
}
