...
// 3. A is loaded with uint4 vectors (24576 B / 16 B = 1536 uint4; threads
//    0..191 load exactly 2 each, no tail access outside A). A misaligned A
//    (not 16-byte aligned) falls back to a scalar shared-memory load; both
//    paths touch only bytes inside A.
//
...
// is uniformly reached. Vector path: indices 2t and 2t+1 < 1536 for t<192,
// exactly covering A. Scalar path: t + 256*j < 12288 for t<256, j<48,
// exactly covering A. After the barrier, sA[0..12287] is fully written.
// A thread with m>=3 or n>=288 exits after the barrier and never reads or
// writes C; each valid (m,n) is produced by exactly one thread, so every C
// element is written exactly once. B and A are never written.
//
// Launch geometry: grid = 4 blocks (blockIdx.x*85 + 85 >= 288 for bx=3:
// columns 255..287 valid), block = 256 threads, dynamic smem = 0 (static
// 24 KiB fits the default limit, no opt-in required).
...
__global__ void __launch_bounds__(BLOCK) gemm3x288_kernel(
  ...
  __shared__ __nv_bfloat16 sA[M * K];   // exactly 24 KiB

  // One-time cooperative staging of all of A into shared memory.
  if (ALIGNED) {
    if (threadIdx.x < 192) {            // 192 * 2 uint4 = 1536 uint4 = 24576 B
      const uint4* src = reinterpret_cast<const uint4*>(A);
      uint4* dst = reinterpret_cast<uint4*>(sA);
      dst[2 * threadIdx.x]     = src[2 * threadIdx.x];
      dst[2 * threadIdx.x + 1] = src[2 * threadIdx.x + 1];
    }
  } else {
