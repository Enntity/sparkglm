...
#define BOTH
#endif
constexpr unsigned M=12, H=8192, V=4096, MODES=4;
BOTH inline uint32_t mix(uint32_t x) {
    x ^= x>>16; x*=0x7feb352du; x ^= x>>15; x*=0x846ca68bu; return x^(x>>16);
}
// Integer construction fixes the ORIGINAL BF16 payload, independent of GPU
// rounding or host FP operations. Twelve nonconstant activation rows; weights
...
BOTH inline uint16_t a_bits(unsigned row,unsigned k,bool tiny) {
    if(tiny) {
        if(k==1)return 0xbf00;
        if(k!=0)return 0;
        const unsigned a=row+1;unsigned e=0;while((1u<<(e+1))<=a)++e;
        return uint16_t(((127u+e)<<7)|((a-(1u<<e))<<(7-e)));
    }
