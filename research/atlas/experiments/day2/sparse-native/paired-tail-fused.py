...
"""Offline 2051-candidate paired gate with separately validated fused merge."""
import argparse
import ctypes
import hashlib
import json
...
def main(args):
    ...
    merge_lib = ctypes.CDLL(str(Path(args.merge).resolve(strict=True)))
    merge_lib.atlas_sparse_merge.argtypes = [ctypes.c_void_p] * 7 + [ctypes.c_uint] * 2 + [ctypes.c_void_p]
    merge_lib.atlas_sparse_merge.restype = ctypes.c_int

    def fused_merge(main_out, tail_out, main_lse, tail_lse, tail_counts, output, out_lse):
        status = merge_lib.atlas_sparse_merge(
            main_out.data_ptr(), tail_out.data_ptr(), main_lse.data_ptr(), tail_lse.data_ptr(),
            tail_counts.data_ptr(), output.data_ptr(), out_lse.data_ptr(),
            main_out.shape[0], HEADS, torch.cuda.current_stream().cuda_stream)
        check(status == 0, f"fused merge wrapper failed with status {status}")
    ...
    emit(stage="start", atlas_sha256=digest(args.atlas), native_sha256=digest(args.native),
         merge_sha256=digest(args.merge),
    ...
    def merge_microgate():
        # Deterministic CPU construction consumes NO fixture RNG draws.
        rows = 12
        counts = torch.tensor([0, 0, 1, 1, 2, 2, 3, 3, 1, 2, 3, 0], dtype=torch.int32)
        l0 = torch.tensor([0, 1000, 500, -500, 1000, -1000, 0, 0, 10, -10, 1.25, -1000], dtype=torch.float32)
        l1 = torch.tensor([float("nan"), float("nan"), -500, 500, 1000, -1000, 1, -1,
                           10.125, -9.875, -1.25, float("nan")], dtype=torch.float32)
        l0, l1 = l0[:, None].expand(rows, HEADS).contiguous(), l1[:, None].expand(rows, HEADS).contiguous()
        elements = torch.arange(rows * HEADS * DIM, dtype=torch.int32).reshape(rows, HEADS, DIM)
        main_cpu = (((elements % 257) - 128).float() / 64).to(torch.bfloat16)
        tail_cpu = ((((elements * 7 + 3) % 263) - 131).float() / 64).to(torch.bfloat16)
        tail_cpu[counts == 0] = float("nan")
        inputs = [x.to("cuda") for x in (main_cpu, tail_cpu, l0, l1, counts)]
        output = torch.full((rows, HEADS, DIM), float("nan"), dtype=torch.bfloat16, device="cuda")
        output_lse = torch.full((rows, HEADS), float("nan"), dtype=torch.float32, device="cuda")
        fused_merge(*inputs, output, output_lse)
        torch.cuda.synchronize()
        actual, actual_lse = output.cpu(), output_lse.cpu()
        check(bool(torch.isfinite(actual).all()) and bool(torch.isfinite(actual_lse).all()),
              "fused microgate: nonfinite output, including poisoned zero-tail rows")
        check(torch.equal(actual[counts == 0], main_cpu[counts == 0]), "zero-tail must copy main exactly")
        check(torch.equal(actual_lse[counts == 0], l0[counts == 0]), "zero-tail LSE must copy main exactly")
        results = {}
        for dtype in (torch.float64, torch.float32):
            has_tail = (counts > 0)[:, None]
            left = l0.to(dtype)
            right = torch.where(has_tail, l1.to(dtype), -float("inf"))
            maximum = torch.maximum(left, right)
            a, b = torch.exp2(left - maximum), torch.exp2(right - maximum)
            denom = a + b
            safe_tail = torch.where(has_tail[..., None], tail_cpu.to(dtype), 0)
            expected = main_cpu.to(dtype) * (a / denom)[..., None] + safe_tail * (b / denom)[..., None]
            expected_lse = maximum + torch.log2(denom)
            # Permit half a local BF16 spacing plus 2e-6 FP32 arithmetic slack
            # against the UNROUNDED oracle, not loose bitwise-equality claims.
            ...
            spacing = torch.maximum(up - rounded.double(), rounded.double() - down)
            diff = (actual.double() - expected.double()).abs()
            check(bool((diff <= spacing * 0.5 + 2e-6).all()), f"fused microgate: {dtype} blend error exceeds BF16 rounding")
            lse_error = float((actual_lse.double() - expected_lse.double()).abs().max())
            check(lse_error <= 1e-4, f"fused microgate: {dtype} LSE error {lse_error}")
            results[str(dtype)] = {"max_abs_unrounded": float(diff.max()), "lse_max_abs": lse_error}
        emit(stage="merge_microgate", passed=True, rows=rows, finite_lse_differences=[-1000, 1000],
             poisoned_zero_tail=True, oracle_errors=results, torch_peak_reserved_bytes=memcheck())
    ...
    def fixture(rows, simple=False):
        ...
        def merge():
            fused_merge(main_out, tail_out, main_lse, tail_lse, tail_counts, nout, lse)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--merge", default="/eval/libatlas_sparse_merge.so")
