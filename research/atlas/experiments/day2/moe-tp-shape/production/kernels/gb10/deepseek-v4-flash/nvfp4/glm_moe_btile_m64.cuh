...
// Original M64 A/scales/MMA order; only B staging uses tiled packed bytes.
// Production SSOT for [N/128,K/64,128,32]; serving remains disabled.
// Include after production grouped kernel primitives.
#pragma once
...
__device__ __forceinline__ void glm_moe_btile_m64_impl(
    ...
    __shared__ unsigned char bt64_Ap[2][M_TILE][K_STEP_T64 / 2 + 16];
    __shared__ unsigned char bt64_As[2][M_TILE][K_STEP_T64 / GROUP_SIZE];
    // Packed B already has MMA-ready rows. Double-buffer the direct stage.
    __shared__ unsigned char bt64_Bp[2][N_TILE_LG][K_STEP_T64 / 2 + 16];
    __shared__ unsigned char bt64_Bs[2][K_STEP_T64 / GROUP_SIZE][N_TILE_LG];
    __shared__ int bt64_tok[M_TILE];

    if (threadIdx.x < M_TILE) {
        const int local_row = threadIdx.x;
        if (sorted_token_ids && (cta_m_local + local_row) < (unsigned int)M_expert)
            bt64_tok[local_row] = sorted_token_ids[cta_m + local_row];
        else
            bt64_tok[local_row] = (int)(cta_m + local_row);
    }
    __syncthreads();
    ...
    #define PQ4_ISSUE_LOADS(buf, kb) do { \
        { \
            ...
            unsigned int a_row = (unsigned int)bt64_tok[row]; \
            moe_cp_async_pred_16(&bt64_Ap[(buf)][row][col], \
        ...
        if constexpr (PQ_VEC_SCALES) { \
            ...
            if (threadIdx.x < M_TILE) { \
                ...
                unsigned int a_row = (unsigned int)bt64_tok[row]; \
                moe_cp_async_pred_4(&bt64_As[(buf)][row][0], \
        ...
        { \
            ...
            for (int rnd = 0; rnd < 2; rnd++) { \
                ...
                moe_cp_async_pred_16(&bt64_Bp[(buf)][bn][bk], \
        ...
        if constexpr (PQ_VEC_SCALES) { \
            ...
            if (threadIdx.x < 32) { \
                ...
                moe_cp_async_pred_16(&bt64_Bs[(buf)][g][ns], \
    ...
    #define PQ4_COMPUTE_MMA(a_buf, b_buf) do { \
        ...
        unsigned int a0 = PQ4_FRAG(bt64_Ap[(a_buf)], ra,     tid * 8); \
        unsigned int a1 = PQ4_FRAG(bt64_Ap[(a_buf)], ra + 8, tid * 8); \
        unsigned int a2 = PQ4_FRAG(bt64_Ap[(a_buf)], ra,     32 + tid * 8); \
        unsigned int a3 = PQ4_FRAG(bt64_Ap[(a_buf)], ra + 8, 32 + tid * 8); \
        unsigned int sfa_m = (lane_id & 1) * 8 + (lane_id >> 2); \
        unsigned int sfa = (unsigned int)bt64_As[(a_buf)][warp_m_offset + sfa_m][0] \
            | ((unsigned int)bt64_As[(a_buf)][warp_m_offset + sfa_m][1] << 8) \
            | ((unsigned int)bt64_As[(a_buf)][warp_m_offset + sfa_m][2] << 16) \
            | ((unsigned int)bt64_As[(a_buf)][warp_m_offset + sfa_m][3] << 24); \
        _Pragma("unroll") \
        for (int nt = 0; nt < 16; nt++) { \
            unsigned int nc = nt * 8 + group_id; \
            unsigned int b0 = PQ4_FRAG(bt64_Bp[(b_buf)], nc, tid * 8); \
            unsigned int b1 = PQ4_FRAG(bt64_Bp[(b_buf)], nc, 32 + tid * 8); \
            unsigned int sfn = nt * 8 + (lane_id >> 2); \
            unsigned int sfb = (unsigned int)bt64_Bs[(b_buf)][0][sfn] \
                | ((unsigned int)bt64_Bs[(b_buf)][1][sfn] << 8) \
                | ((unsigned int)bt64_Bs[(b_buf)][2][sfn] << 16) \
                | ((unsigned int)bt64_Bs[(b_buf)][3][sfn] << 24); \
...
#define GLM_GU_BTILE_M64_CALL(VEC) \
    glm_moe_gate_up_btile_m64_impl<VEC>(A_packed, A_scale, gate_packed_ptrs, gate_scale_ptrs, \
...
extern "C" __global__ void glm_moe_gate_up_btile_m64(GLM_GU_BTILE_M64_ARGS) {
    GLM_GU_BTILE_M64_CALL(false);
}
extern "C" __global__ void glm_moe_gate_up_btile_m64_vecscale(GLM_GU_BTILE_M64_ARGS) {
    GLM_GU_BTILE_M64_CALL(true);
}
