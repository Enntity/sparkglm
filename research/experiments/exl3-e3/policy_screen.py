...
p.add_argument('--cap', type=int, choices=(32,64,128), default=128)
p.add_argument('--iterations', type=int, default=40)
p.add_argument('--repeats', type=int, default=7)
a = p.parse_args()
output = Path(a.output)
if output.exists(): raise SystemExit('output exists')
r = dict(schema='sparkglm.e3-policy-screen/v1', status='running', cap=a.cap, results=[],
         source_sha256={f.name:hashlib.sha256(f.read_bytes()).hexdigest()
                        for f in Path(__file__).parent.glob('*.py')})
try:
    if a.cap != 128:
        boundary = case([0,1,a.cap-1,a.cap,a.cap+1,63,64,65], 20260908, a.iterations, a.repeats, a.cap)
        boundary['routing'] = 'threshold-boundaries'
        r['results'].append(boundary)
    ...
    for tokens in (2048, 4096, 7168):
        ...
        for name, counts in [('balanced', balanced), ('skewed', skewed)]:
            result = case(counts, 20260908, a.iterations, a.repeats, a.cap, tokens)
            result['routing'] = name
