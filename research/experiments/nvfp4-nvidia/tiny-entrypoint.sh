...
if [[ "${SPARKGLM_MODELOPT_TINY:-0}" == "1" ]]; then
  [[ -f /usr/local/lib/python3.12/dist-packages/sparkglm_modelopt_tiny.py && -f /usr/local/lib/python3.12/dist-packages/sparkglm_modelopt_tiny.pth ]] || {
...
if [[ "${SPARKGLM_MODELOPT_TINY:-0}" == "1" ]]; then
  python3 -S - "${MODEL_DIR}/config.json" <<'PYSAFE'
import json, sys
if json.load(open(sys.argv[1])).get("_sparkglm_fixture") != "tinyglm-modelopt-v1":
    raise SystemExit("NVFP4 dummy loading requires the synthetic fixture")
PYSAFE
  [[ "${SPEC_METHOD:-dflash}" == "none" && "${QUANTIZATION:-exl3}" == "modelopt_fp4" ]] || { log "invalid NVFP4 fixture options"; exit 1; }
  args+=(--load-format dummy --generation-config vllm)
fi
